...
|---|---|---|
| ```aleph_rproxy (Reverse Proxy)``` | Acceso | Único punto de entrada entre entre el cliente y el sistema. Funciona como un intermediario entre el usuario y los componentes de presentación. Este se encarga de recibir las peticiones de los clientes y enviarlas, a través de los componentes de presentación y comunicación, a los componentes internos de la aplicación. Permite: proteger el sistema interno y hacer uso de SSL/HTTPS. |
...
|```aleph_ap_load_balancer (Load Balancer API Gateway)```| Comunicación |Se encarga de la distribución del tráfico entrante hacia alguna de las instancias de API Gateway disponibles. El balanceo está determinado por el algoritmo *Least Connection* (dirección del tráfico al servidor con menos conexiones).|
...
| ```aleph_rproxy```   | ```aleph_ag_lb```   |HTTPS|Enrutamiento de solicitudes web externas desde el reverse proxy hacia el balanceador del API Gateway.|
| ```aleph_rproxy_dsk```   | ```aleph_ag_lb```   |HTTPS|Enrutamiento de solicitudes del escritorio del usuario externas desde el reverse proxy hacia el balanceador del API Gateway.|
