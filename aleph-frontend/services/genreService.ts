...
export async function getGenreDetails(genreSlug: string): Promise<GenreDetails> {
  try {
    ...
    const genreResponse = await authHttpClient.get(`${MUSIC_API_URL}/genres/slug/${genreSlug}`);
    ...
    const songsResponse = await authHttpClient.get(`${MUSIC_API_URL}/songs?genre=${encodeURIComponent(genreData.name)}`);
